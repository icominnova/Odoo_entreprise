...
class AppointmentController(http.Controller):
    ...
    def appointment_update_available_slots(self, appointment_type_id, staff_user_id=None, resource_selected_id=None, asked_capacity=1, timezone=None, **kwargs):
        ...
        if appointment_type.category != 'custom':
            now = datetime.now()
            if appointment_type.category == 'punctual' and appointment_type.start_datetime and appointment_type.start_datetime > now:
                base_date = appointment_type.start_datetime
            elif appointment_type.min_schedule_hours:
                base_date = now + relativedelta(hours=appointment_type.min_schedule_hours)
            else:
                base_date = now
            asked_month = base_date + relativedelta(months=int(kwargs.get('month_id', 0)))

        slots = appointment_type.with_context(
            appointment_slots_force_month=(asked_month.month, asked_month.year) if asked_month else False
